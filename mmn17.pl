...
test0(ToPos):-
    ...
    check_player_choise(4,3,6,5,Pos,ToPos).
...
check_player_choise(FromRow,FromCol,ToRow,ToCol,pos(w,WList,BList),pos(b,NWList,NBList)):-
    ...
    call(Tool),!,
    (   canMove(Tool,tool(w,_Type,ToRow,ToCol)),!
    ;  canEat(Tool,tool(w,_Type,ToRow,ToCol),_)),
   % retract(Tool),
...
canEat(Tool,FinalPos):-
  canEat(Tool,FinalPos,0).

canEat(Tool,FinalPos,EatenNum):-
    Tool = tool(C1,_,_,_),
    call(Tool),
    ...
    canEat(NPos,FinalPos,NEatenNum).
canEat(Tool,Tool,EatenNum):-
    EatenNum>0.

canMove(tool(C,minion,Row,Col),tool(C,Type,NRow,NCol)):-
    tool(C,minion,Row,Col),
    ...
    ;   Type = minion),
    retract(tool(C,minion,Row,Col)),
    asserta(tool(C,Type,NRow,NCol)).
...
canMove(tool(Color,queen,Row,Col),tool(Color,queen,NRow,NCol)):-
    ...
    retract(tool(Color,queen,Row,Col)),
    asserta(tool(Color,queen,NRow,NCol)).
...
%    call(T),
    ...
    retractall(tool(_,_,_,_)),
    assertTools(WTools,BTools).
move(pos(C1,WTools,BTools),pos(C2,NWTools,NBTools)):-
 %   retractall(tool(_,_,_,_)),
  %  assertTools(WTools,BTools),
    T = tool(C1,minion,_,_),
  %  call(T),
    color(C1,C2),
    canMove(T,tool(C1,queen,Row,Col)),
  %  retract(T),
   % assert(NT),
   % findall(tool(w,T1,R1,Col1),tool(w,T1,R1,Col1),NewWTools),
    (   (   C1 = w,
            append([tool(C1,queen,Row,Col)],WTools,NWTools),
            NBTools = BTools
        )
    ;   (   C1 = b,
        append([tool(C1,queen,Row,Col)],BTools,NBTools),
        NWTools = WTools
    )),
    retractall(tool(_,_,_,_)),
    assertTools(WTools,BTools).
move(pos(C1,WTools,BTools),pos(C2,NWTools,NBTools)):-
  %  retractall(tool(_,_,_,_)),
   % assertTools(WTools,BTools),
    ...
    color(C1,C2),
 %   call(T),
    canMove(T,tool(C1,T,Row,Col)),
 %   retract(T),
%    assert(NT),
  %  findall(tool(C1,T2,R2,Col2),tool(C2,T2,R2,Col2),NBTools),
    (   (   C1 = w,
            append([tool(C1,T,Row,Col)],WTools,NWTools),
            NBTools = BTools
        )
    ;   (   C1 = b,
        append([tool(C1,T,Row,Col)],BTools,NBTools),
        NWTools = WTools
    )),
    retractall(tool(_,_,_,_)),
    assertTools(WTools,BTools).
...
moves(pos(C,WTools,BTools),PosList):-
    retractall(tool(_,_,_,_)),
    assertTools(WTools,BTools),
    findall(ToPos,move(pos(C,WTools,BTools),ToPos),PosList).
